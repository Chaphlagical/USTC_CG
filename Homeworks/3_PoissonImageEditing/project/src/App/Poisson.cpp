...
void Poisson::GetPoisson(QPoint paste_point, QPoint source_point, cv::Mat& paste_img_, cv::Mat& source_img_)
{
	...
	source_point_ = source_point;
	//cv::Mat Poisson_mask(width_, height_,CV_8UC3);
	clock_t t = clock();
...
//	div_.clear();
	...
	std::cout << "1 "<< clock() - t << std::endl;
	t = clock();


	//Eigen::VectorXf vec_b(pixels_num_);
	...
	for (int j = 0; j < width_; j++)
	{
		for (int k = 0; k < height_; k++)
		{
			if (inside_mask_(j, k) == 1)
			{
				...
				/*if (vec_x(index) > 255)
				{
					paste_img_.at<cv::Vec3b>(j+ paste_point_.y(), k+ paste_point_.x())[i] = 255;
				}
				else if (vec_x(index) < 0)
				{
					paste_img_.at<cv::Vec3b>(j+ paste_point_.y(), k+ paste_point_.x())[i] = 0;
				}
				else
				{
					paste_img_.at<cv::Vec3b>(j+ paste_point_.y(), k+ paste_point_.x())[i] = vec_x(index);
				}*/
					
			}
		}
	}
	std::cout <<"3 "<< clock() - t << std::endl;
	t = clock();
}
